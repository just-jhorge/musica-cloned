...
const Player = () => {
  ...
  const loop = () => {
    document.querySelectorAll(".repeath path").forEach((item) => {
  ...
  const handleClick = (e) => {
    const selectedLength = e.clientX - volumeWrapper.offsetLeft;
    setVolume((selectedLength / volumeWrapper.clientWidth).toFixed(2));
  };
    ...
    <div className="fixed bottom-0 bg-[rgba(29,33,35,0.3)] border-t border-white/[0.1] backdrop-blur-lg py-4 lg:py-6 w-full pl-6 -ml-6 lg:ml-0 z-50">
      ...
      <div className="container mx-auto flex items-center justify-between">
        ...
        <div className="flex flex-col md:w-3/5 mx-9 grow-0">
          <div className="flex items-center justify-center md:mb-8">
            <svg
              width="16"
              height="17"
              className="shuffle hidden md:inline-block h-4 mr-11 play-icon"
              onClick={shuffle}
              viewBox="0 0 16 17"
              fill="none"
            ...
            <img
              src={previous}
              onClick={prevTrack}
              className="hidden md:inline-block h-4 mr-11 play-icon"
            />
            <div
              onClick={togglePlay}
              className="mr-6 md:mr-11 h-[25px] w-[25px] text-white text-xs rounded-full bg-primary-yellow shadow-[0_0_18px_rgba(255,255,255,0.3)] flex items-center justify-center"
            >
              {audioPlayer.paused ? (
                <svg
                  width="9"
            ...
            <img
              src={nextIcon}
              onClick={nextTrack}
              className="h-4 md:mr-11 hover:cursor-pointer play-icon"
            />
            <svg
              width="16"
              className="repeat hidden md:inline-block h-4 fill-primary-yellow play-icon"
              onClick={loop}
              height="17"
              viewBox="0 0 16 17"
              xmlns="http://www.w3.org/2000/svg"
          ...
          <div className="hidden md:block h-1 bg-white/[0.04] w-full rounded relative">
            ...
            />
            <div
              className="bg-primary-yellow h-full rounded player"
        ...
        <div className="hidden md:flex items-center w-1/5 grow-0">
          ...
          <div
            className="volume-wrapper h-1 bg-white/[0.04] w-full rounded hover:cursor-pointer"
            onClick={handleClick}
          >
            <div
